...
const init = () => {
  // Lock grid items to fixed order (prevents visual reordering)
  document.querySelectorAll('.grid__item').forEach((item, index) => {
    item.style.order = String(index);
  });

  // Attach click handlers to grid items (exclude video items - they play inline)
  document.querySelectorAll('.grid__item:not([data-video])').forEach((item) => {
...
// Extract image URL, video src, and caption text from a grid item
const extractItemData = (item) => {
  const imgDiv = item.querySelector('.grid__item-image');
  const caption = item.querySelector('figcaption');
  const isVideo = item.classList.contains('grid__item--video');
  const videoSrc = item.dataset.videoSrc;
  const posterSrc = item.dataset.posterSrc || (imgDiv && imgDiv.style.backgroundImage);
  const imgURL = imgDiv?.style?.backgroundImage || (posterSrc && `url(${posterSrc})`);
  return {
    isVideo: !!isVideo && !!videoSrc,
    videoSrc: videoSrc || null,
    imgURL: imgURL || (posterSrc ? `url(${posterSrc})` : 'none'),
    title: caption?.querySelector('h3')?.textContent || '',
    desc: caption?.querySelector('p')?.textContent || '',
  };
};

// Set the panel's background, video, and text based on clicked item
const setPanelContent = ({ isVideo, videoSrc, imgURL, title, desc }) => {
  const panelImg = panel.querySelector('.panel__img');
  const panelVideoWrap = panel.querySelector('.panel__video-wrap');
  const panelVideo = panel.querySelector('.panel__video');
  if (isVideo && videoSrc && panelVideoWrap && panelVideo) {
    panelImg.style.backgroundImage = 'none';
    panelImg.style.display = 'none';
    panelVideoWrap.classList.add('is-active');
    panelVideo.src = videoSrc;
    panelVideo.load();
  } else {
    panelVideoWrap?.classList.remove('is-active');
    panelVideo?.pause();
    panelVideo?.removeAttribute('src');
    panelImg.style.display = '';
    panelImg.style.backgroundImage = imgURL || 'none';
  }
    ...
    .fromTo(
      ...
      { y: 25 },
      {
        ...
        onComplete: () => {
          ...
          isPanelOpen = true;
          const panelVideo = panel.querySelector('.panel__video');
          const isVideoPanel = panel.querySelector('.panel__video-wrap')?.classList.contains('is-active');
          if (isVideoPanel && panelVideo?.src) {
...
const resetView = () => {
  ...
  const panelVideoWrap = panel.querySelector('.panel__video-wrap');
  if (panelVideoWrap) panelVideoWrap.classList.remove('is-active');
  const panelImg = panel.querySelector('.panel__img');
  if (panelImg) panelImg.style.display = '';

  const allItems = document.querySelectorAll('.grid__item');
  const delays = computeStaggerDelays(currentItem, allItems);
  const panelMedia = panel.querySelector('.panel__media') || panel.querySelector('.panel__img');
